    ...
    <div className="space-y-10 animate-slide-up max-w-7xl">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight">Performance <span className="text-indigo-300 underline decoration-indigo-500/20 underline-offset-8">Analytics</span></h1>
          <p className="text-slate-500 font-medium max-w-xl">
            {activeProject ? (
              <>Synthesizing data for <span className="text-indigo-300 font-black">{activeProject.name}</span>. Deep-dive into velocity metrics and defect density.</>
            ) : (
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchAnalytics} disabled={loading}
            className="group flex items-center gap-2 px-6 py-3 rounded-2xl bg-transparent/0 border border-slate-200 shadow-sm hover:border-indigo-500/30 hover:bg-slate-50 transition-all">
            <RefreshCw className={cn("h-4 w-4 text-slate-400 group-hover:text-indigo-500 transition-colors", loading && "animate-spin")} />
            <span className="text-xs font-black uppercase tracking-widest text-slate-600 group-hover:text-indigo-300">Sync Intelligence</span>
          </button>
        </div>
      </div>
            ...
            {[
              {
                ...
                iconCls: "bg-emerald-50 text-emerald-300 border-emerald-100", 
              ...
              },
              {
                ...
                iconCls: "bg-rose-50 text-rose-300 border-rose-100", 
              ...
              },
              {
                ...
                iconCls: "bg-indigo-50 text-indigo-300 border-indigo-100", 
                ...
                <div className="flex items-start justify-between mb-6">
                  ...
                  {card.trend && (
                    <span className={cn("text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg",
                      card.up ? "bg-emerald-50 text-emerald-300" : "bg-rose-50 text-rose-300"
                    )}>
                      {card.trend}
                    </span>
                  )}
              ...
              {data.weeklyData.map((day, i) => (
                <div key={i} className="flex-1 flex flex-col gap-4 group">
                  ...
                  <div className="p-2 rounded-xl bg-slate-50 group-hover:bg-indigo-50 border border-slate-100 group-hover:border-indigo-100 transition-all">
                    <p className="text-center text-[10px] font-black text-slate-600 group-hover:text-indigo-300 uppercase tracking-widest">{day.label}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="premium-card p-8 space-y-8">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={item.label} className="p-4 rounded-[1.5rem] bg-slate-50/50 border border-slate-100 flex items-center justify-between group hover:bg-transparent/0 hover:shadow-lg hover:shadow-slate-200/50 transition-all">
                    <div className="flex items-center gap-3">
              ...
              <div className="grid grid-cols-2 gap-3 pt-4 border-t border-slate-100">
                {[
                  { label: "OPEN",     count: data.bugStats.open,       cls: "bg-rose-50 text-rose-300 border-rose-100" },
                  { label: "PROGRESS", count: data.bugStats.inProgress, cls: "bg-amber-50 text-amber-300 border-amber-100" },
                  { label: "RESOLVED", count: data.bugStats.resolved,   cls: "bg-emerald-50 text-emerald-300 border-emerald-100" },
            ...
            <div className="premium-card p-8 space-y-8">
              ...
              {data.upcomingDeadlines.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-emerald-50/20 rounded-[2.5rem] border border-emerald-100/30">
                  ...
                  <p className="text-[10px] font-black text-emerald-300 uppercase tracking-widest">No Critical Deadlines</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {data.upcomingDeadlines.map((task) => (
                    <div key={task.id} className="group p-5 rounded-[2rem] bg-transparent/0 border border-slate-200 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-500/5 transition-all relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-start gap-4">
                        ...
                        <Badge className={cn("font-black text-[9px] uppercase tracking-widest px-2",
                          task.priority === "CRITICAL" ? "bg-rose-50 text-rose-300 border-rose-100" : 
                          task.priority === "HIGH" ? "bg-amber-50 text-amber-300 border-amber-100" : 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                <div key={i} className={cn("p-8 rounded-[2.5rem] text-center border transition-all hover:scale-105", 
                  metric.met ? "bg-slate-50/50 border-slate-100" : "bg-transparent/0 border-slate-200"
                )}>
                  ...
                  <div className="flex items-center justify-center gap-2 mt-4">
                    ...
                    <span className={cn("text-[10px] font-black uppercase tracking-widest", metric.met ? "text-emerald-300" : "text-rose-300")}>
