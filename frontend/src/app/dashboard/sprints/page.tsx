  ...
  return (
    <div className="space-y-10 animate-slide-up max-w-4xl">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <Button onClick={() => setShowCreate(true)} variant="premium" className="shadow-violet-500/20">
      ...
      {sprints.length === 0 ? (
        <div className="premium-card p-24 text-center border-dashed">
          ...
          <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Static Pipeline</h3>
          ...
          <Button onClick={() => setShowCreate(true)} variant="premium" className="mt-8 !rounded-2xl">
            INITIALIZE SPRINT
          </Button>
        </div>
      ) : (
        <div className="space-y-4">
          {sprints.map((sprint, i) => {
            ...
            return (
              <div key={sprint.id} className="premium-card !p-0 overflow-hidden group transition-all hover:shadow-xl hover:shadow-indigo-500/5">
                <div className="p-8">
                    ...
                    <div className="flex-1 min-w-0 space-y-4">
                      ...
                      {sprint.goal && (
                        <div className="flex items-center gap-3">
                          ...
                          <p className="text-sm font-medium text-slate-600">{sprint.goal}</p>
                        </div>
                      )}
                      ...
                      <div className="flex flex-wrap items-center gap-6">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{sprintTasks.length} OBJECTS</span>
                        </div>
                      </div>
                    </div>
                  ...
                  {sprintTasks.length > 0 && (
                    <div className="mt-8 space-y-3">
                      <div className="flex justify-between items-end">
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Throughput Velocity</span>
                ...
                {isExpanded && (
                  <div className="border-t border-slate-50 p-8 bg-slate-50/50 space-y-4">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-6">Assigned work items</h4>
                    {sprintTasks.length === 0 ? (
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-widest text-center py-4">Backlog empty</p>
                    ) : (
                        ...
                        {sprintTasks.map((task) => (
                          <div key={task.id} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-4 shadow-sm">
                            <div className={cn("w-2 h-2 rounded-full shrink-0",
                              ...
                              task.status === "testing" ? "bg-amber-500" : "bg-slate-200"
                            )} />
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="sm:max-w-[550px] !rounded-[2.5rem] p-0 overflow-hidden border-none shadow-2xl">
          ...
          <div className="p-10 space-y-8 max-h-[60vh] overflow-y-auto no-scrollbar">
            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Cycle Designation *</Label>
              <Input value={formName} onChange={(e) => setFormName(e.target.value)}
                placeholder="e.g. Iteration 04 — Core Infrastructure" className="h-14 !rounded-2xl bg-slate-50 border-slate-200 font-bold" />
            </div>
            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Strategic Goal</Label>
              <Textarea value={formGoal} onChange={(e) => setFormGoal(e.target.value)}
                placeholder="What is the primary objective of this cycle?" rows={3} className="!rounded-2xl bg-slate-50 border-slate-200 font-medium" />
            </div>
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Commencement</Label>
                <Input type="date" value={formStart} onChange={(e) => setFormStart(e.target.value)} className="h-12 !rounded-xl bg-slate-50 border-slate-200 font-bold" />
              </div>
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Termination</Label>
                <Input type="date" value={formEnd} onChange={(e) => setFormEnd(e.target.value)} className="h-12 !rounded-xl bg-slate-50 border-slate-200 font-bold" />
              </div>
            </div>
          ...
          <div className="p-10 bg-slate-50 flex items-center justify-between border-t border-slate-200">
            <button onClick={() => setShowCreate(false)} className="text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-colors">Discard</button>
            <Button onClick={handleCreate} disabled={!formName.trim()} variant="premium" className="!h-14 !px-10 shadow-indigo-500/20">
