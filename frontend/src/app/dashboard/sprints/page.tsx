...
import { Zap, Plus, Calendar, Target, CheckCircle2, Clock, Play, Trash2, ChevronDown, ChevronUp, Layers, Edit3, RotateCcw } from "lucide-react";
...
export default function SprintsPage(){
  ...
  const resetForm=()=>{setFormName("");setFormGoal("");setFormStart("");setFormEnd("");setFormTaskIds([]);};
  const handleCreate=()=>{
  ...
  const openEdit=(sprint:Sprint)=>{setEditSprint(sprint);setFormName(sprint.name);setFormGoal(sprint.goal);setFormStart(sprint.startDate);setFormEnd(sprint.endDate);setFormTaskIds(sprint.taskIds);};
  const handleEdit=()=>{
    if(!editSprint||!formName.trim()||!activeProjectId)return;
    const updated=sprints.map((s)=>s.id===editSprint.id?{...s,name:formName.trim(),goal:formGoal.trim(),startDate:formStart,endDate:formEnd,taskIds:formTaskIds}:s);
    setSprints(updated);saveSprints(activeProjectId,updated);setEditSprint(null);resetForm();
  };
  ...
  const TaskForm=({color="violet"}:{color?:string})=>(<>{projectTasks.length>0&&(<div className="space-y-3"><div className="flex items-center justify-between"><Label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Assign Tasks</Label><span className={`text-[10px] font-bold text-${color}-600 bg-${color}-50 px-2 py-0.5 rounded-full`}>{formTaskIds.length} selected</span></div><div className="space-y-1.5 max-h-44 overflow-y-auto pr-1 no-scrollbar">{projectTasks.map((task)=>(<label key={task.id} className={`flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-${color}-200 hover:bg-white cursor-pointer transition-all`}><input type="checkbox" checked={formTaskIds.includes(task.id)} onChange={()=>toggleTask(task.id)} className={`w-4 h-4 rounded border-slate-300 text-${color}-600`}/><span className={cn("w-2 h-2 rounded-full shrink-0",task.status==="done"?"bg-emerald-500":task.status==="in-progress"?"bg-violet-500":task.status==="testing"?"bg-amber-500":task.status==="todo"?"bg-blue-500":"bg-slate-300")}/><span className="text-xs font-semibold text-slate-800 flex-1 truncate">{task.title}</span><span className="text-[9px] text-slate-400 font-medium capitalize shrink-0">{task.priority}</span></label>))}</div></div>)}</>);
  return(
    <div className="space-y-8 animate-slide-up max-w-4xl">
                ...
                <div className="p-6 sm:p-7">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {sprint.status==="planned"&&<button type="button" onClick={(e)=>{e.preventDefault();e.stopPropagation();handleStatus(sprint.id,"active");}} style={{transform:"none",cursor:"pointer"}} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition-colors"><Play className="h-3.5 w-3.5"/>Start</button>}
                      {sprint.status==="active"&&<button type="button" onClick={(e)=>{e.preventDefault();e.stopPropagation();handleStatus(sprint.id,"completed");}} style={{transform:"none",cursor:"pointer"}} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold transition-colors"><CheckCircle2 className="h-3.5 w-3.5"/>Complete</button>}
                      {sprint.status==="completed"&&<button type="button" onClick={(e)=>{e.preventDefault();e.stopPropagation();handleStatus(sprint.id,"active");}} style={{transform:"none",cursor:"pointer"}} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-600 hover:bg-slate-700 text-white text-xs font-bold transition-colors"><RotateCcw className="h-3.5 w-3.5"/>Reopen</button>}
                      <button onClick={()=>openEdit(sprint)} style={{transform:"none"}} title="Edit sprint" className="w-9 h-9 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:border-indigo-200 transition-colors"><Edit3 className="h-4 w-4"/></button>
                ...
                {exp&&<div className="border-t border-slate-100 bg-slate-50/60 p-6 space-y-3">
                  ...
                  {st.length===0?<p className="text-sm text-slate-400 text-center py-4">No tasks assigned to this sprint.</p>:(
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {st.map((task)=>(<div key={task.id} className="bg-white p-3.5 rounded-xl border border-slate-100 flex items-center gap-3 shadow-sm"><span className={cn("w-2 h-2 rounded-full shrink-0",task.status==="done"?"bg-emerald-500":task.status==="in-progress"?"bg-violet-500":task.status==="testing"?"bg-amber-500":task.status==="todo"?"bg-blue-500":"bg-slate-300")}/><span className={cn("text-xs font-semibold flex-1 truncate",task.status==="done"?"text-slate-400 line-through":"text-slate-800")}>{task.title}</span><span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider capitalize shrink-0">{task.status.replace("-"," ")}</span></div>))}
                    </div>
                  )}
                </div>}
              </div>
      ...
      <Dialog open={showCreate} onOpenChange={(open)=>{if(!open){setShowCreate(false);resetForm();}}}>
        <DialogContent className="sm:max-w-[540px] !rounded-[2rem] p-0 overflow-hidden border-none shadow-2xl">
          <div className="bg-violet-600 px-8 py-7 flex items-center gap-5"><div className="w-12 h-12 rounded-2xl bg-white/20 flex items-center justify-center shrink-0"><Layers className="h-6 w-6 text-white"/></div><div><DialogTitle className="text-2xl font-black text-white tracking-tight">New Sprint</DialogTitle><DialogDescription className="text-white/70 text-sm mt-0.5">Add a sprint to <span className="text-white font-bold">{activeProject?.name}</span></DialogDescription></div></div>
          <div className="px-8 py-7 space-y-5 max-h-[65vh] overflow-y-auto no-scrollbar">
            ...
            {projectTasks.length>0&&(<div className="space-y-3"><div className="flex items-center justify-between"><Label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Assign Tasks</Label><span className="text-[10px] font-bold text-violet-600 bg-violet-50 px-2 py-0.5 rounded-full">{formTaskIds.length} selected</span></div><div className="space-y-1.5 max-h-44 overflow-y-auto pr-1 no-scrollbar">{projectTasks.map((task)=>(<label key={task.id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-violet-200 hover:bg-white cursor-pointer transition-all"><input type="checkbox" checked={formTaskIds.includes(task.id)} onChange={()=>toggleTask(task.id)} className="w-4 h-4 rounded border-slate-300 text-violet-600"/><span className={cn("w-2 h-2 rounded-full shrink-0",task.status==="done"?"bg-emerald-500":task.status==="in-progress"?"bg-violet-500":task.status==="testing"?"bg-amber-500":task.status==="todo"?"bg-blue-500":"bg-slate-300")}/><span className="text-xs font-semibold text-slate-800 flex-1 truncate">{task.title}</span><span className="text-[9px] text-slate-400 font-medium capitalize shrink-0">{task.priority}</span></label>))}</div></div>)}
          </div>
          <div className="px-8 py-5 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
            <button onClick={()=>{setShowCreate(false);resetForm();}} className="text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors">Cancel</button>
      ...
      {/* Edit Sprint Dialog */}
      <Dialog open={!!editSprint} onOpenChange={(open)=>{if(!open){setEditSprint(null);resetForm();}}}>
        <DialogContent className="sm:max-w-[540px] !rounded-[2rem] p-0 overflow-hidden border-none shadow-2xl">
          <div className="bg-indigo-600 px-8 py-7 flex items-center gap-5"><div className="w-12 h-12 rounded-2xl bg-white/20 flex items-center justify-center shrink-0"><Edit3 className="h-6 w-6 text-white"/></div><div><DialogTitle className="text-2xl font-black text-white tracking-tight">Edit Sprint</DialogTitle><DialogDescription className="text-white/70 text-sm mt-0.5">Update sprint details</DialogDescription></div></div>
          <div className="px-8 py-7 space-y-5 max-h-[65vh] overflow-y-auto no-scrollbar">
            <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Sprint Name <span className="text-rose-400">*</span></Label><Input value={formName} onChange={(e)=>setFormName(e.target.value)} className="h-12 rounded-2xl bg-slate-50 border-slate-200 font-semibold"/></div>
            <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Sprint Goal <span className="text-slate-300 normal-case font-normal">(optional)</span></Label><Textarea value={formGoal} onChange={(e)=>setFormGoal(e.target.value)} rows={2} className="rounded-2xl bg-slate-50 border-slate-200 font-medium text-sm resize-none"/></div>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Start Date</Label><Input type="date" value={formStart} onChange={(e)=>setFormStart(e.target.value)} className="h-11 rounded-xl bg-slate-50 border-slate-200 font-semibold"/></div>
              <div className="space-y-1.5"><Label className="text-[10px] font-black uppercase tracking-wider text-slate-400">End Date</Label><Input type="date" value={formEnd} onChange={(e)=>setFormEnd(e.target.value)} className="h-11 rounded-xl bg-slate-50 border-slate-200 font-semibold"/></div>
            </div>
            {projectTasks.length>0&&(<div className="space-y-3"><div className="flex items-center justify-between"><Label className="text-[10px] font-black uppercase tracking-wider text-slate-400">Assign Tasks</Label><span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">{formTaskIds.length} selected</span></div><div className="space-y-1.5 max-h-44 overflow-y-auto pr-1 no-scrollbar">{projectTasks.map((task)=>(<label key={task.id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-indigo-200 hover:bg-white cursor-pointer transition-all"><input type="checkbox" checked={formTaskIds.includes(task.id)} onChange={()=>toggleTask(task.id)} className="w-4 h-4 rounded border-slate-300 text-indigo-600"/><span className={cn("w-2 h-2 rounded-full shrink-0",task.status==="done"?"bg-emerald-500":task.status==="in-progress"?"bg-violet-500":task.status==="testing"?"bg-amber-500":task.status==="todo"?"bg-blue-500":"bg-slate-300")}/><span className="text-xs font-semibold text-slate-800 flex-1 truncate">{task.title}</span><span className="text-[9px] text-slate-400 font-medium capitalize shrink-0">{task.priority}</span></label>))}</div></div>)}
          </div>
          <div className="px-8 py-5 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
            <button onClick={()=>{setEditSprint(null);resetForm();}} className="text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors">Cancel</button>
            <button onClick={handleEdit} disabled={!formName.trim()} style={{transform:"none"}} className="h-11 px-8 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm shadow-lg disabled:opacity-40 flex items-center gap-2 transition-colors"><CheckCircle2 className="h-4 w-4"/>Save Changes</button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
